...
class Netra {
  constructor() {
    ...
    // single source of truth used to guard against calling start() while
    // it's already running, which throws "recognition has already
    // started" (InvalidStateError) and was spamming the console/killing
    // wake-word detection.
  ...
  setupEventListeners() {
    ...
    const mainMicButton = document.getElementById("mainButton");
    if (mainMicButton) {
  ...
  initSpeechRecognition() {
    ...
    this.commandRecognition.onstart = () => {
      ...
      // NOTE: previously spoke "Speak your command now" here, but onstart
      // fires AFTER recognition has already begun listening - meaning the
      // microphone was guaranteed to hear that exact prompt play through
      // the speakers, especially on a laptop without headphones. That
      // prompt is now spoken BEFORE recognition starts instead, in
      // startVoiceCommand(), so the mic isn't listening while it plays.
    };
    ...
    this.commandRecognition.onresult = (event) => {
      ...
      if (command.length > 2) {
        // Show command in UI
        this.showRecognizedCommand(command);
        this.routeVoiceCommand(command).finally(() => {
          this.resumeDetectionAfterInteraction();
        });
      } else {
        console.log(
        ...
        // Nothing was actually asked - resume detection rather than
    ...
    this.commandRecognition.onerror = (event) => {
      ...
      // If recognition fails outright, don't leave detection paused -
      // resume it rather than waiting on a command that will never come.
      this.resumeDetectionAfterInteraction();
    ...
    this.commandRecognition.onend = () => {
      ...
      // Safety net - if the recognition session ended without onresult or
      // onerror ever firing (can happen on some browsers), make sure
      // detection doesn't stay paused forever. Idempotent, so harmless if
      // it already resumed via one of those paths.
      this.resumeDetectionAfterInteraction();
  ...
  initContinuousListening() {
    ...
    this.continuousRecognition.onstart = () => {
      this.continuousRecognitionRunning = true;
    };
  ...
  startVoiceCommand() {
    ...
    try {
      ...
      // Only show feedback if this wasn't triggered by volume button to
      // avoid conflicts. Speak the prompt BEFORE recognition starts (not
      // after, as it previously did) and wait for it to actually finish
      // speaking before opening the microphone - otherwise the mic is
      // guaranteed to hear this exact prompt play back through the
      ...
      if (!this.volumeUpPressed) {
        ...
        // Small initial delay to let speechSynthesis actually register
        // as speaking before we start polling isSpeaking.
        setTimeout(waitForPromptThenListen, 150);
      } else {
  ...
  startContinuousListening() {
    if (
      ...
      try {
        ...
        // Optimistically mark as running immediately (onstart will
        // confirm this shortly after). This closes the race where two
        // callers both check the flag before onstart has had a chance
        // to fire.
        this.continuousRecognitionRunning = true;
      } catch (error) {
        console.log("Continuous listening start error:", error.message);
        // start() threw, so it's definitely not running - make sure the
        // flag reflects reality instead of getting stuck "true".
        if (error && error.name === "InvalidStateError") {
          // It was already running - trust that state, not our flag.
          this.continuousRecognitionRunning = true;
        } else {
          this.continuousRecognitionRunning = false;
        }
  ...
  stopContinuousListening() {
    if (this.continuousRecognition) {
      ...
      // stop() is async (onend fires later) but nothing else should try
      // to start() until onend actually confirms it's stopped, so don't
      // flip continuousRecognitionRunning here - let onend do it.
    }
  }
  ...
  handleWakeWordDetected(commandAfterWake = null) {
    ...
    if (commandAfterWake) {
      ...
      this.routeVoiceCommand(commandAfterWake).finally(() => {
        this.resumeDetectionAfterInteraction();
      });
      return;
    }
   ...
   * whether detection was actually running beforehand.
   */
  pauseDetectionForInteraction() {
    if (this._interactionPauseActive) return; // already paused, don't overwrite the original state
    this._interactionPauseActive = true;
    this.wasDetectingBeforePause = this.isDetecting;
    if (this.isDetecting) {
      this.isDetecting = false; // stops the detection loop from continuing
    }
  }
   ...
   */
  async resumeDetectionAfterInteraction() {
    if (!this._interactionPauseActive) return; // we didn't pause it, don't touch it
    this._interactionPauseActive = false;

    if (!this.wasDetectingBeforePause) return; // wasn't running before, leave it off

    const maxWaitMs = 15000; // safety cap so we never wait forever
    const start = Date.now();
    while (this.isSpeaking && Date.now() - start < maxWaitMs) {
  ...
  showRecognizedCommand(command) {
    ...
    try {
      ...
      if (!commandDisplay) {
        ...
        const statusArea = statusEl ? statusEl.parentElement : document.body;
        if (statusArea) {
      ...
      if (commandDisplay) {
        ...
        if (commandTextEl) commandTextEl.textContent = command;
  ...
  async captureAndReadText() {
    ...
    // Pause detection's own announcements while capturing and reading -
    // the camera stream itself stays active (needed to actually capture
    // the frame), only the announcement loop pauses.
    this.pauseDetectionForInteraction();
    ...
    canvas.toBlob(async (blob) => {
      ...
      try {
        ...
        // Always resume, whether the read succeeded, found nothing, or
        // failed outright - never leave detection paused indefinitely.
        await this.resumeDetectionAfterInteraction();
      }
    }, "image/jpeg");
  }
